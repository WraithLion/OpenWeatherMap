...
#Filtra el archivo usando sólo los vuelos en origen y destino
...
filtrarVuelosRepetidos <- function(archi){
    #Revisa que cada renglón de las columnas con "origin" y "destination" si estan repetidas
...
#Filtra todos los vuelos, tanto de origen como destino para facilitar la petición del clima
unirFiltrarLista <- function(ciudadesSinRepeticion){
    ...
    #Usando la tabla, se realiza de nuevo un filtro para que los elementos repetidos sólo se muestren una vez
...
#Función que crea una tabla para poder añadir la información del clima de cada ciudad
#Parametros
#Vuelos, es el archivo que tiene la información de la anterior función
...
#Función que obtiene realiza la petición para obtener los datos del clima
